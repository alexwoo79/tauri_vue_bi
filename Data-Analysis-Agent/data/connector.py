...
class MultiDataSource(DataSource):
    """
    Combine multiple DataSource instances into a single shared SQLite DB.
    Each source's tables are loaded under a unique prefix to avoid collisions.
    """

    def __init__(self):
        self.name = "MultiDataSource"
        self._conn = sqlite3.connect(":memory:", check_same_thread=False)
        self._tables: List[str] = []

    def add_source(self, source: DataSource) -> None:
        """Copy all tables from *source* into the shared DB with a prefixed name."""
        safe_prefix = _clean_identifier(source.name.rsplit(".", 1)[0]) or "file"
        # Collect all tables from the source's own connection (file-based sources)
        src_conn = getattr(source, "_conn", None)
        if src_conn is not None:
            cur = src_conn.cursor()
            cur.execute("SELECT name FROM sqlite_master WHERE type='table' ORDER BY rowid")
            for (tname,) in cur.fetchall():
                df = pd.read_sql_query(f'SELECT * FROM "{tname}"', src_conn)
                dest_table = _clean_identifier(f"{safe_prefix}_{tname}") or f"tbl{len(self._tables)}"
                # Deduplicate if same prefix+table already loaded
                base = dest_table
                idx = 1
                while dest_table in self._tables:
                    dest_table = f"{base}_{idx}"
                    idx += 1
                df.to_sql(dest_table, self._conn, if_exists="replace", index=False)
                self._tables.append(dest_table)
                log.info("[MultiDS] added table %r  (%d rows)", dest_table, len(df))
    ...
    def get_schema(self) -> str:
        ...
        for table in self._tables:
            try:
                ...
                parts.append(_table_schema_str(self._conn, table, cnt))
            except Exception:
                pass
