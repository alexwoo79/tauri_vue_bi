...
from LLM.llm_config_manager import LLMConfig
...
@bp.post("/api/models/sync")
def sync_model_config():
    """Upsert model config from desktop app (provider id is supplied by frontend)."""
    d = request.json or {}
    provider = d.get("provider", "").strip()
    api_key = d.get("api_key", "").strip()
    if not provider or not api_key:
    ...
    if provider in config_manager.DEFAULT_CONFIGS:
        ok = config_manager.set_config(
            provider,
            api_key,
            base_url=base_url,
        ...
        if not ok:
            return jsonify({"error": f"同步失败: {provider}"}), 400
        return jsonify({"ok": True, "provider": provider})

    existing = config_manager.get_config(provider)
    config_manager.configs[provider] = LLMConfig(
        provider=provider,
        api_key=api_key,
        base_url=base_url or (existing.base_url if existing else None),
        model=model or (existing.model if existing else provider),
        enabled=enabled,
        is_custom=True,
        context_window=context_window,
        max_output_tokens=max_output_tokens,
