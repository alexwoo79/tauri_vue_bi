...
from data.connector import ExcelDataSource, CSVDataSource, SQLDataSource, GoogleSheetsDataSource, HTTPAPIDataSource, MultiDataSource
...
def upload_file(sid: str):
    ...
    try:
        ...
        if append and sess.data_source is not None:
            # Merge into MultiDataSource
            if isinstance(sess.data_source, MultiDataSource):
                multi = sess.data_source
            else:
                multi = MultiDataSource()
                multi.add_source(sess.data_source)
            multi.add_source(source)
            sess.data_source = multi
        else:
            sess.data_source = source
        schema = sess.data_source.get_schema()
