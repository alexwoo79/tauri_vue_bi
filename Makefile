...
.PHONY: help install dev build bundle dmg test test-rust test-ts lint fmt clean \
	check-deps icon update-deps \
	release-check release-tag release-tag-fix release-push release
...
help:
	...
	@printf "  make bundle       完整打包桌面应用\n"
	@printf "  make dmg          仅构建 macOS DMG 安装包\n"
	@printf "  make release TAG=tauri-vue-bi-v0.1.0  发布检查 + 打 tag + 推送\n"
