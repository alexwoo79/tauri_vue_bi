...
impl ToolCallBuilder {
    ...
    fn build(self) -> crate::agent::llm::client::ToolCall {
        crate::agent::llm::client::ToolCall {
            id: self.id,
            function: crate::agent::llm::client::ToolCallFunction {
                name: self.name,
                        ...
                        if let Ok(stream_resp) = serde_json::from_str::<StreamResponse>(json_data) {
                            if let Some(choice) = stream_resp.choices.first() {
                                ...
                                if choice.finish_reason == Some("tool_calls".to_string()) {
                                    let final_tool_calls: Vec<crate::agent::llm::client::ToolCall> = tc_accumulator
                                        .values()
                                        .map(|b| b.clone().build())
                                        .collect();
