// src-tauri/src/agent/functions/charts/stacked_area_chart.rs
//
// 堆叠面积图生成模块

use anyhow::{Context, Result};
use plotly::{Plot, Layout, Scatter};
use plotly::common::{Title, Mode, Fill};
use serde_json::json;
...
pub fn generate_stacked_area_chart(
    ...
    for (i, (series_name, (x_vals, y_vals))) in groups.iter().enumerate() {
        ...
        let trace = Scatter::new(x_vals.clone(), y_vals.clone())
            .mode(Mode::Lines)
            .fill(Fill::ToZeroY)
            .name(series_name)
...
<body>
    <div class="chart-header">
        ...
        <div class="chart-info">ID: stacked_area | Engine: plotly | Format: x列(类别) + 分组列 + y列(数值)</div>
    </div>
    <div class="chart-container">{chart_html}</div>
</body>
</html>"#,
        title = options.title.clone().unwrap_or("堆叠面积图".to_string()),
