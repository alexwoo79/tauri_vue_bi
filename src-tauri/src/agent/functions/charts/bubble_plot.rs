// src-tauri/src/agent/functions/charts/bubble_plot.rs
//
// 气泡图生成模块
...
pub fn generate_bubble_plot(
    ...
    // ====================== 终极修复 ======================
    let min_size = size_values.iter().fold(f64::INFINITY, |a, &b| a.min(b));
    let max_size = size_values.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
    let size_range = max_size - min_size;

    let pixel_sizes: Vec<usize> = size_values
        .iter()
        .map(|&s| {
            if size_range == 0.0 {
                30
            } else {
                let normalized = (s - min_size) / size_range;
                (normalized * 50.0 + 10.0) as usize
            }
        })
        .collect();
    ...
    let trace = Scatter::new(x_values, y_values)
        .mode(Mode::Markers)
        .name(&options.title.clone().unwrap_or("气泡图".to_string()))
        .marker(
            plotly::common::Marker::new()
                .size_array(pixel_sizes)
...
<body>
    <div class="chart-header">
        ...
        <div class="chart-info">ID: bubble_plot | Engine: plotly | Format: x列(数值) + y列(数值) + size列(数值)</div>
    </div>
    <div class="chart-container">{chart_html}</div>
</body>
</html>"#,
        title = options.title.clone().unwrap_or("气泡图".to_string()),
