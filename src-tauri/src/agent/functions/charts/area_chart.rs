// src-tauri/src/agent/functions/charts/area_chart.rs
//
// 面积图生成模块
...
pub fn generate_area_chart(
    ...
    let trace = Scatter::new(x_values, y_values)
        .mode(Mode::Lines)
        .fill(Fill::ToZeroY)
        .name(&options.title.clone().unwrap_or("面积图".to_string()))
...
<body>
    <div class="chart-header">
        ...
        <div class="chart-info">ID: area_chart | Engine: plotly | Format: x列(类别) + y列(数值)</div>
    </div>
    <div class="chart-container">{chart_html}</div>
</body>
</html>"#,
        title = options.title.clone().unwrap_or("面积图".to_string()),
