...
pub fn run_decile_analysis(
    ...
    let filtered_values: Vec<f64> = filtered_indices
        .iter()
    ...
    let mut result_df = DataFrame::new(vec![decile_series, filtered_series])?;
    ...
    let breakdown_df = if let Some(group_col) = groupby_column {
        ...
        let filtered_group_values: Vec<String> = filtered_indices
            .iter()
...
fn compute_stats(df: &DataFrame, target_col: &str) -> Result<DataFrame> {
    ...
    let mut stats_df = DataFrame::new(vec![
        Series::new("decile".into(), deciles).into(),
        Series::new("count".into(), counts).into(),
        Series::new("sum".into(), sums.clone()).into(),
    ...
    let cumulative_pct: Vec<f64> = pct_of_total
        .iter()
        .map(|p| {
            cumulative_sum += p;
            cumulative_sum
...
fn generate_insight(stats: &DataFrame, target_column: &str, n_deciles: usize) -> String {
    ...
    writeln!(
        markdown,
        "2. **均值分布**: 最高均值出现在第 1 桶，最低均值出现在第 {} 桶",
        n_deciles
    )
    .unwrap();
