...
pub fn run_decision_tree(
    ...
    let target_series = target_col.as_series().unwrap_or_else(|| Series::new("", Vec::<f64>::new()));
    let target_casted = target_series.cast(&DataType::Float64).unwrap_or_else(|_| Series::new("", Vec::<f64>::new()));
    
    for feature in &features {
        let feature_col = df.column(feature)?;
        let feature_series = feature_col.as_series().unwrap_or_else(|| Series::new("", Vec::<f64>::new()));
        let feature_casted = feature_series.cast(&DataType::Float64).unwrap_or_else(|_| Series::new("", Vec::<f64>::new()));
    ...
    for (i, (feature, &importance)) in features.iter().zip(importance_values.iter()).enumerate() {
        if importance > 0.1 {
            ...
            rule_texts.push(format!("如果 {} 较高，则目标值倾向于较高 (重要性: {:.2})", feature, importance));
        }
    }
...
/// 生成分析洞察文本
fn generate_insight(importance_df: &DataFrame, target_column: &str, max_depth: usize) -> Result<String> {
    ...
    let mut markdown = format!("## 决策树分析结果\n\n### 概览\n- **目标变量**: {}\n- **最大深度**: {}\n\n### 重要特征\n", target_column, max_depth);
