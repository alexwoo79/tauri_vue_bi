...
pub fn fill_missing_values(
    ...
    for &col in columns {
        ...
        let series = col_ref.as_series().ok_or_else(|| anyhow::anyhow!("Failed to get series"))?;
...
/// 查找众数
fn find_mode(series: &Series) -> Option<AnyValue> {
    let vc = match series.value_counts(false, false, None) {
        Ok(v) => v,
        Err(_) => return None,
    };
    if vc.height() == 0 {
...
pub fn drop_missing_rows(df: &DataFrame, columns: Option<&[&str]>) -> Result<DataFrame> {
    match columns {
        Some(cols) => df.drop_nulls(Some(cols)),
        None => df.drop_nulls(None),
